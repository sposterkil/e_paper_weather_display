...
def get_weather(lat, lon, api_key, units="imperial"):
    ...
    return { 
        ...
        "current_conditions": ", ".join([condition["main"] for condition in body["current"]["weather"]]),
        "forecast": [(day["temp"]["max"], day["temp"]["min"]) for day in body["daily"]][:3] # > [(279.4, 273.15), (279.4, 273.15)]
    }

def get_now_playing():
    tn = telnetlib.Telnet("localhost", 9090)
    tn.write("status".encode("ascii")+b"\n")
    status_output = tn.read_all().decode("ascii")
    tn.close()
